# unlearn/generate_finetune_csv_from_draws.py
#
# Builds a summary CSV for the 8 extended fine-tune configs (budget 300/400
# + seed-repeats) purely from the raw per-draw .npy arrays that
# average_finetune_draws_extended.py already saved to results/. Does NOT
# load any model, checkpoint, or baseline.pt — reads only:
#   - data/capability_taxonomy.json          (tier map)
#   - data/baseline_per_class_acc.npy         (cached baseline accuracy)
#   - results/finetune_{tag}_steps{N}_draws_extended.npy  (per config)
#
...
#   results/finetune_results_from_saved_draws_steps<N>.csv
#
# Run from the repo root: python unlearn/generate_finetune_csv_from_draws.py
...
def build_configs():
    """
    Same 8 configs as average_finetune_draws_extended.py's build_configs(),
...
def main(n_steps=N_STEPS):
    ...
    out_path = f'results/finetune_results_from_saved_draws_steps{n_steps}.csv'
    fieldnames = [
