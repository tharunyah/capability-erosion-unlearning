"""
audit/capability_monitor.py  (PATCHED)
...
1. FIXED: history contamination. check() previously called
   self._update_history() unconditionally on every call, including for runs
   being evaluated (e.g. attack runs). This meant checking attack #1 would
   leak into the baseline used to judge attack #2, drifting the "normal"
   mean toward attack-like values as you go through a loop.
   -> check() no longer updates history at all. History is ONLY ever
      updated via seed_benign_history() (or the new add_benign_observation()),
      i.e. only from runs you explicitly know are benign ground truth.
      This makes the calibration set stable regardless of what/how many
      runs you evaluate afterward.

2. FIXED (silent failure): min_history_for_zscore defaulted to 5, but if
   you only have 3 benign samples per method (as in this project's
   forget_random_{50,100,200} setup), len(past_shifts) never reaches 5 --
   so Check B silently never runs, for any method, forever. There was no
   warning; it just always fell through to "ok" unless Check A fired.
   -> min_history_for_zscore is still a parameter (default now 3, matching
      typical project sample counts -- change it if you add more benign
      draws), and check() now WARNS the first time it skips Check B due to
      insufficient history, so this can't silently happen again unnoticed.

Note: even with both fixes, 3 benign samples is a small basis for a z-score.
Consider using generate_random_repeats.py-style additional random-seed draws
to get 5+ benign samples per method/tier before trusting Check B's output.
"""
...
class CapabilityMonitor:
    def __init__(self, di_threshold=0.8, zscore_threshold=2.0, min_history_for_zscore=3):
        """
        ...
        min_history_for_zscore: need at least this many past (benign) di_shift
                            ...
                            NOTE: default changed from 5 to 3 to match this
                            project's typical benign sample count (3 random
                            forget sets per budget sweep). If you generate
                            more benign draws (recommended), raise this back
                            up -- 3 is a bare minimum, not an ideal.
        """
    ...
    def seed_benign_history(self, di_shift_list, tier):
        ...
        real/attack runs. This is the ONLY way history gets populated now.
        """
    ...
    def add_benign_observation(self, di_shift, tier):
        """
        Add a single known-benign di_shift to history. Use this if you want
        to incrementally grow calibration data (e.g. after generating and
        evaluating an additional random-seed benign draw) without re-seeding
        the whole batch.
        """
