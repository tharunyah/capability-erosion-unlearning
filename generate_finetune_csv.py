...
def main():
    ...
    # ── Baseline ──────────────────────────────────────────────────────────
    # Reuses the same cached baseline_per_class_acc.npy that generate_fisher_csv.py
    # writes — if that file already exists from the Fisher run, this recomputes it
    # anyway (cheap, keeps this script runnable standalone) and overwrites in place.
    print("\nEvaluating baseline...")
    ...
    for strategy in strategies:
        for budget in budgets:
            ckpt_path = f'models/unlearned_finetune_{strategy}_{budget}.pt'
            print(f"\nEvaluating {ckpt_path}...")

            model = load_model(ckpt_path, device)
            save_path = f'data/unlearned_finetune_{strategy}_{budget}_per_class_acc.npy'
    ...
    # ── Write CSV ─────────────────────────────────────────────────────────
    out_path = 'results/finetune_results.csv'
    fieldnames = [
